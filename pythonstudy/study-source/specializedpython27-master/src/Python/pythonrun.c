    ...
    if (f == NULL || f == Py_None)
            ...
            PyObject_HasAttrString(value, "print_file_and_line"))
        {
            ...
            if (!parse_syntax_error(value, &message, &filename,
                ...
                if (filename == NULL)
                    PyFile_WriteString(MANGELED_EVAL_STRING, f);
                else
                  ...
                  PyObject *locals, PyCompilerFlags *flags)
{
    ...
    mod = PyParser_ASTFromString(str, MANGELED_EVAL_STRING, start, flags, arena);
    if (mod != NULL)
        ret = run_mod(mod, MANGELED_EVAL_STRING, globals, locals, flags, arena);
    PyArena_Free(arena);
    return ret;
}
