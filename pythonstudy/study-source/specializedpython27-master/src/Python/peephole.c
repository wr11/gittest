...
#define FOLDABLE_UNARY_OP(op) (op==UNARY_NEGATIVE || op==UNARY_CONVERT \
    || op==UNARY_INVERT)
    ...
    for (i=0 ; i<codelen ; i += CODESIZE(codestr[i])) {
      reoptimize_current:
        ...
        switch (opcode) {
            ...
            case LOAD_FAST:
                j = GETARG(codestr, i);
                /* LOAD_FAST 0 LOAD_CONST c --> LOAD_FAST_ZERO_LOAD_CONST c.
                   1. The argument for LOAD_FAST must be 0.
                   2. The blocks must be the same(the LOAD_CONST is not a
                      lable for any jump instruction).
                   3. The opcode after LOAD_CONST must NOT be another
                      LOAD_CONST or UNARY_OP, so that we won't break any
                      optimiation for LOAD_CONST op. */
                if (j == 0 &&
                    codestr[i+3] == LOAD_CONST &&
                    codestr[i+6] != LOAD_CONST &&
                    !FOLDABLE_UNARY_OP(codestr[i+6]) &&
                    ISBASICBLOCK(blocks, i, 6)) {
