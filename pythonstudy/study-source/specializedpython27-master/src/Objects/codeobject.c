           ...
           PyObject *lnotab)
{
    ...
    if (co != NULL) {
        ...
        Py_INCREF(code);
        co->co_code = code;
		co->co_firstinstr = (_Py_CODEUNIT *)PyString_AS_STRING(code);
        Py_INCREF(consts);
        co->co_consts = consts;
        Py_INCREF(names);
        co->co_names = names;
...
code_dealloc(PyCodeObject *co)
{
	...
	if (co->co_quickened) {
		const _PyEntryZero *zero = &co->co_quickened[0].entry.zero;
		int count = zero->cache_count + (zero->instruction_count + INSTRUCTIONS_PER_ENTRY - 1) / INSTRUCTIONS_PER_ENTRY;
