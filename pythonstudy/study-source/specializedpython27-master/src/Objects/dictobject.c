...
static PyObject *dummy = NULL; /* Initialized by first call to newPyDictObject() */
                               ...
                               /* Object used as ma_lookup result to represent an entry is not found */
static PyDictEntry _missing_entry;
static PyDictEntry *missing_entry = &_missing_entry;

/* entryop codes for ma_lookup functions */
#define ENTRY_NOP       0  /* Nothing is to be done to the entry */
#define ENTRY_ADD       1  /* Add an entry if one is not found */
#define ENTRY_REMOVE   -1  /* Remove the entry if one is found */

/* Special values in index table */
#define INDEX_EMPTY    -1  /* An emptry slot. Don't change this value. */
#define INDEX_REMOVED  -2  /* An index has been removed from this slot. (not used currently) */
#endif
...
#ifdef Py_COMPACT_DICT
static PyDictEntry *
lookdict_string(PyDictObject *mp, PyObject *key, long hash, int entryop);
#else
static PyDictEntry *
...
#define INIT_NONZERO_DICT_SLOTS(mp) do {                                \
    ...
    (mp)->ma_index = (mp)->ma_smallindex;                               \
    memset((mp)->ma_smallindex, -1, sizeof((mp)->ma_smallindex));       \
    (mp)->ma_mask = PyDict_MINSIZE - 1;                                 \
    (mp)->ma_allocated = PyDict_MINENTRIES;                             \
    } while(0)
...
PyDict_New(void)
{
    ...
    if (dummy == NULL) { /* Auto-initialize dummy */
        ...
        if (dummy == NULL)
            return NULL;
#ifdef Py_COMPACT_DICT
        memset(missing_entry, 0, sizeof(PyDictEntry));
#endif
...
#ifdef Py_COMPACT_DICT
        assert(mp->ma_index == mp->ma_smallindex);
        assert(mp->ma_allocated == PyDict_MINENTRIES);
#endif
...
#define FETCH_INDEX(ip0, slot, indexwidth, index) do {                  \
    switch (indexwidth) {                                               \
    case 0:                                                             \
        index = ((int8_t *)ip0)[slot]; break;                           \
    case 1:                                                             \
        index = ((int16_t *)ip0)[slot]; break;                          \
    case 2:                                                             \
        index = ((int32_t *)ip0)[slot]; break;                          \
    case 3:                                                             \
        index = ((int64_t *)ip0)[slot]; break;                          \
    default:                                                            \
        assert(0);                                                      \
    }                                                                   \
} while (0)

#define STORE_INDEX(ip0, slot, indexwidth, index) do {                  \
    switch (indexwidth) {                                               \
    case 0:                                                             \
        ((int8_t *)ip0)[slot] = (int8_t)index; break;                   \
    case 1:                                                             \
        ((int16_t *)ip0)[slot] = (int16_t)index; break;                 \
    case 2:                                                             \
        ((int32_t *)ip0)[slot] = (int32_t)index; break;                 \
    case 3:                                                             \
        ((int64_t *)ip0)[slot] = (int64_t)index; break;                 \
    default:                                                            \
        assert(0);                                                      \
    }                                                                   \
} while (0)
...
/* This alghorithm is from the list_resize function of PyListObject.
...
*/
#define ENTRYTABLE_SIZE(use, allocation, indexwidth) do {               \
    allocation = ((use) >> 3) + ((use) < 9 ? 3 : 6);                        \
                                                                        \
    /* check for integer overflow */                                    \
    if (allocation > PY_SIZE_MAX - (use)) {                               \
        allocation = -1;                                                \
    }                                                                   \
    else {                                                              \
        allocation += (use);                                              \
    }                                                                   \
    if (allocation <= INT8_MAX)                                         \
        indexwidth = 0;                                                 \
    else if (allocation <= INT16_MAX)                                   \
        indexwidth = 1;                                                 \
    else if (allocation <= INT32_MAX)                                   \
        indexwidth = 2;                                                 \
    else                                                                \
        indexwidth = 3;                                                 \
} while (0)
...
/*
Append new entry with zero values to the entry table.
This may lead to a change of indexwidth, in which case the
index table will be updated to accommodate the new indexwidth.
*/
int entrytable_append(PyDictObject *mp)
{
    size_t new_allocated;
    PyDictEntry *oldtable = mp->ma_table;
    int is_oldtable_malloced;
    PyDictEntry *newtable;
    size_t fill = mp->ma_fill;
    size_t mask = mp->ma_mask;
    size_t i, slot, ei;
    int8_t *indextable, *new_indextable;
    int indexwidth, new_indexwidth;

    if ((size_t)(mp->ma_allocated) > fill) {
        memset(&oldtable[fill], 0, sizeof(PyDictEntry));
        return 0;
    }

    ENTRYTABLE_SIZE(fill + 1, new_allocated, new_indexwidth);
    if (new_allocated == -1) {
        PyErr_NoMemory();
        return -1;
    }
    ...
    assert(new_allocated > PyDict_MINENTRIES);
    /* Allocate a new entrytable. If alloc-strategy is modified,
    do not forget to update the rollback logic in this function
    when indextable allocation failed. */
    if (is_oldtable_malloced) {
        newtable = PyMem_RESIZE(oldtable, PyDictEntry, new_allocated);
        if (newtable == NULL) {
            PyErr_NoMemory();
            return -1;
        }
        ...
        newtable = (PyDictEntry *)PyMem_MALLOC(new_allocated * sizeof(PyDictEntry));
        if (newtable == NULL) {
            PyErr_NoMemory();
            return -1;
        }
        memcpy(newtable, oldtable, fill * sizeof(PyDictEntry));
    }
    memset(&newtable[fill], 0, sizeof(PyDictEntry));

    /* Update index width */
    indexwidth = mp->ma_indexwidth;
    assert(new_indexwidth >= indexwidth);
    if (new_indexwidth > indexwidth) {
        new_indextable = PyMem_NEW(int8_t, (mask + 1) << new_indexwidth);
        if (new_indextable == NULL) {
            /* Rollback all changes */
            if (is_oldtable_malloced)
                mp->ma_table = PyMem_RESIZE(newtable, PyDictEntry, mp->ma_allocated);
            else
                PyMem_DEL(newtable);
            PyErr_NoMemory();
            return -1;
        }
        memset(new_indextable, -1, (mask + 1) << new_indexwidth);

        indextable = mp->ma_index;
        i = fill;
        for (slot = 0; i > 0; slot++) {
            FETCH_INDEX(indextable, slot, indexwidth, ei);
            if (ei != INDEX_EMPTY) {
                i--;
                STORE_INDEX(new_indextable, slot, new_indexwidth, ei);
            }
        }

        if (indextable != (int8_t*)mp->ma_smallindex)
            PyMem_DEL(indextable);
        mp->ma_index = new_indextable;
        mp->ma_indexwidth = new_indexwidth;
    }

    mp->ma_table = newtable;
    mp->ma_allocated = new_allocated;
    return 0;
}
...
#ifdef Py_COMPACT_DICT
    /*
    * Note that when entryop is ENTRY_ADD, a entrytable_append may be triggered
    * by this function to add a new entry, and the indexwidth may have to change
    * while the index table have to be updated to accommodate the new index width.
    */
    , int entryop
#endif
    )
{
    ...
    PyObject *startkey;
#ifdef Py_COMPACT_DICT
    void *ip0 = mp->ma_index;
    size_t ei;
    int indexwidth = mp->ma_indexwidth;

#define CHECK_ADD_ENTRY_RETURN(slot) do {                       \
    if (entryop == ENTRY_ADD) {                                 \
        if (entrytable_append(mp) == -1)                        \
            return NULL;                                        \
        ei = mp->ma_fill;                                       \
        ip0 = mp->ma_index;                                     \
        indexwidth = mp->ma_indexwidth;                         \
        ep0 = mp->ma_table;                                     \
        STORE_INDEX(ip0, slot, indexwidth, ei);                 \
        return &ep0[ei];                                        \
    }                                                           \
} while (0)
    ...
    i = (size_t)hash & mask;
#ifdef Py_COMPACT_DICT
    FETCH_INDEX(ip0, i, indexwidth, ei);
    if (ei == INDEX_EMPTY) {
        CHECK_ADD_ENTRY_RETURN(i);
    ...
    if (ep->me_key == dummy)
        ...
        if (ep->me_hash == hash) {
            ...
            if (ep0 == mp->ma_table && ep->me_key == startkey) {
                 ...
                 */
#ifdef Py_COMPACT_DICT
                return lookdict(mp, key, hash, entryop);
#else
    ...
    for (perturb = hash; ; perturb >>= PERTURB_SHIFT) {
        i = (i << 2) + i + perturb + 1;
#ifdef Py_COMPACT_DICT
        FETCH_INDEX(ip0, i & mask, indexwidth, ei);
        if (ei == INDEX_EMPTY) {
            if (freeslot)
                return freeslot;
            CHECK_ADD_ENTRY_RETURN(i & mask);
            return missing_entry;
        }
        ep = &ep0[ei];
#else
...
#ifdef Py_COMPACT_DICT
    ep = mp->ma_lookup(mp, key, hash, ENTRY_ADD);
#else
...
static void
insertdict_clean(register PyDictObject *mp, PyObject *key, long hash,
    PyObject *value, Py_ssize_t entryindex)
{
    register size_t i;
    register size_t perturb;
    register size_t mask = (size_t)mp->ma_mask;
    ...
    void *ip0 = mp->ma_index;
    register size_t ei;
    register size_t indexwidth = mp->ma_indexwidth;

    MAINTAIN_TRACKING(mp, key, value);
    i = hash & mask;

    FETCH_INDEX(ip0, i, indexwidth, ei);
    for (perturb = hash; ei != INDEX_EMPTY; perturb >>= PERTURB_SHIFT) {
        i = (i << 2) + i + perturb + 1;
        FETCH_INDEX(ip0, i & mask, indexwidth, ei);
    }
    STORE_INDEX(ip0, i & mask, indexwidth, entryindex);
...
#ifndef Py_COMPACT_DICT
#define DICT_RESIZE(mp, minused, minentries) dictresize((mp), (minused))
/*
...
#else
#define DICT_RESIZE(mp, minused, minentries) dictresize((mp), (minused), (minentries))
/*
Restructure the table by allocating a new table and reinserting all
items again.  When entries have been deleted, the new table may
actually be smaller than the old one.
The new index table size will be POT and larger than minused,
while the new entry table size will be larger than minentries.
*/
static int
dictresize(PyDictObject *mp, Py_ssize_t minused, int minentries)
{
    Py_ssize_t newsize;
    void *oldindex = mp->ma_index;
    void *smallindex = mp->ma_smallindex;
    void *newindex;
    int indexwidth = mp->ma_indexwidth;
    int new_indexwidth;
    size_t old_allocated, new_allocated;
    int need_shrink_table = 0;
    int need_move_entries = 1;
    Py_ssize_t i, newei;
    PyDictEntry *ep, *oldtable = mp->ma_table;
    PyDictEntry *smalltable = mp->ma_smalltable;
    PyDictEntry *newtable;
    int is_oldindex_malloced;
    int is_oldtable_malloced;
    ...
    is_oldindex_malloced = oldindex != smallindex;
    assert(oldtable != NULL);
    is_oldtable_malloced = oldtable != smalltable;

    indexwidth = mp->ma_indexwidth;
    ENTRYTABLE_SIZE(minentries, new_allocated, new_indexwidth);
    if (new_allocated == -1) {
        PyErr_NoMemory();
        return -1;
    }
    ...
    if (newsize == PyDict_MINSIZE) {
        ...
        newindex = PyMem_NEW(int8_t, newsize << new_indexwidth);
        if (newindex == NULL) {
            PyErr_NoMemory();
            return -1;
        }
    ...
    if (is_oldtable_malloced) {
        ...
        if (mp->ma_fill == mp->ma_used) need_move_entries = 0;
    }
    else if (new_allocated > PyDict_MINENTRIES) {
        newtable = PyMem_NEW(PyDictEntry, new_allocated);
    }
    else {
        newtable = oldtable;
        if (mp->ma_fill == mp->ma_used) need_move_entries = 0;
    ...
    /* Make the dict empty, using the new table. */
    mp->ma_index = newindex;
    mp->ma_indexwidth = new_indexwidth;
    mp->ma_table = newtable;
    mp->ma_mask = newsize - 1;
    mp->ma_allocated = new_allocated;
    memset(newindex, -1, newsize << new_indexwidth);
    ...
    /* Copy the data over; this is refcount-neutral for active entries;
    dummy entries aren't copied over, of course */
    if (need_move_entries) {
        for (ep = oldtable; i > 0; ep++, i--) {
            if (ep->me_value != NULL) {             /* active entry */
                newtable[newei] = *ep;
                insertdict_clean(mp, ep->me_key, (long)ep->me_hash,
                    ep->me_value, newei);
        ...
        for (ep = oldtable; newei < i; ep++, newei++) {
            assert(ep->me_value != NULL);
            insertdict_clean(mp, ep->me_key, (long)ep->me_hash,
                ep->me_value, newei);
        }
    ...
    if (need_shrink_table)
        /* We assume a shrinking realloc can never fail. */
        mp->ma_table = PyMem_RESIZE(newtable, PyDictEntry, new_allocated);

    if (is_oldindex_malloced)
        PyMem_DEL(oldindex);
    return 0;
}
#endif
...
_PyDict_NewPresized(Py_ssize_t minused)
{
    ...
    if (minused>PyDict_MINENTRIES && op != NULL && DICT_RESIZE((PyDictObject *)op, minused, minused) == -1) {
        Py_DECREF(op);
        return NULL;
    }
    return op;
...
PyDictEntry *
_PyDict_GetEntryPoint(PyObject *op, PyObject *key)
{
	long hash;
...
#ifdef Py_COMPACT_DICT
		ep = (mp->ma_lookup)(mp, key, hash, ENTRY_NOP);
#else
		ep = (mp->ma_lookup)(mp, key, hash);
#endif
		/* ignore errors */
...
{
	PyDictEntry *ep = _PyDict_GetEntryPoint(op, key);
	return ep == NULL ? NULL : ep->me_value;
}
                               ...
                               long hash, PyDictEntry *ep, PyObject *value)
{
    ...
    if (!(mp->ma_used > n_used && mp->ma_fill*3 >= (mp->ma_mask+1)*2))
        return 0;
    return DICT_RESIZE(mp, (mp->ma_used > 50000 ? 2 : 4) * mp->ma_used, mp->ma_used);
}
...
PyDict_DelItem(PyObject *op, PyObject *key)
{
    ...
    mp = (PyDictObject *)op;
#ifdef Py_COMPACT_DICT
    ep = (mp->ma_lookup)(mp, key, hash, ENTRY_REMOVE);
#else
    ep = (mp->ma_lookup)(mp, key, hash);
#endif
    if (ep == NULL)
...
PyDict_Clear(PyObject *op)
{
    ...
    Py_ssize_t fill;
#ifdef Py_COMPACT_DICT
    PyDictEntry small_copy[PyDict_MINENTRIES];
#else
    PyDictEntry small_copy[PyDict_MINSIZE];
#endif
#ifdef Py_DEBUG
     ...
     */
#ifdef Py_COMPACT_DICT
    if (mp->ma_index != mp->ma_smallindex)
...
#ifdef Py_COMPACT_DICT
    mask = ((PyDictObject *)op)->ma_fill - 1;
#else
...
dict_fromkeys(PyObject *cls, PyObject *args)
{
    ...
    if (PyDict_CheckExact(d) && ((PyDictObject *)d)->ma_used == 0) {
        if (PyDict_CheckExact(seq)) {
            ...
            if (DICT_RESIZE(mp, Py_SIZE(seq) / 2 * 3, Py_SIZE(seq))) {
                Py_DECREF(d);
                return NULL;
            }
        ...
        if (PyAnySet_CheckExact(seq)) {
            ...
            if (DICT_RESIZE(mp, PySet_GET_SIZE(seq) / 2 * 3, PySet_GET_SIZE(seq))) {
                Py_DECREF(d);
                return NULL;
            }
...
PyDict_Merge(PyObject *a, PyObject *b, int override)
{
    ...
    if (PyDict_Check(b)) {
        ...
        if ((mp->ma_fill + other->ma_used)*3 >= (mp->ma_mask+1)*2) {
           if (DICT_RESIZE(mp, (mp->ma_used + other->ma_used)*2, mp->ma_used + other->ma_used) != 0)
               return -1;
        }
#ifdef Py_COMPACT_DICT
...
#ifdef Py_COMPACT_DICT
    /* Lookup and remove the entry from the index table. */
    if ((mp->ma_lookup)(mp, ep->me_key, (long)ep->me_hash, ENTRY_REMOVE) != ep)
        assert(0);
#endif
...
#ifdef Py_COMPACT_DICT
    if (mp->ma_table != mp->ma_smalltable)
        res = res + mp->ma_allocated * sizeof(PyDictEntry);
    if (mp->ma_index != mp->ma_smallindex)
        res = res + (mp->ma_mask + 1) * (1 << mp->ma_indexwidth);
#else
    if (mp->ma_table != mp->ma_smalltable)
