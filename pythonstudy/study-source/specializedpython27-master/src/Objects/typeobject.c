...
#define NEOX_PYTHON_CACHE_OPTIMIZE
#ifdef NEOX_PYTHON_CACHE_OPTIMIZE
#define MCACHE_MAX_ATTR_SIZE    100
#define MCACHE_SIZE_EXP         15
#define MCACHE_FORCE_ON
// #define MCACHE_MURMUR_HASH
#else
#define MCACHE_MAX_ATTR_SIZE    100
#define MCACHE_SIZE_EXP         10
#endif

#ifdef MCACHE_MURMUR_HASH
static inline unsigned int MCACHE_HASH_METHOD(PyTypeObject *type, PyObject *name) {
	unsigned int h, k;
	h = 1234;
	k = ((PyStringObject *)(name))->ob_shash * 0xcc9e2d51;
	k = (k << 15) | (k >> 17);
	k *= 0x1b873593;
	h ^= k;
	h = (h << 13) | (h >> 19);
	h = (h * 5) + 0xe6546b64;
	k = type->tp_version_tag * 0xcc9e2d51;
	k = (k << 15) | (k >> 17);
	k *= 0x1b873593;
	h ^= k;
	h = (h << 13) | (h >> 19);
	h = (h * 5) + 0xe6546b64;
	return h & ((1 << MCACHE_SIZE_EXP) - 1);
}
#else
...
type_new(PyTypeObject *metatype, PyObject *args, PyObject *kwds)
{
	...
	if (base->tp_flags & Py_TPFLAGS_ENABLE_ADAPTIVE_OPCODE)
		type->tp_flags |= Py_TPFLAGS_ENABLE_ADAPTIVE_OPCODE;
	...
	{
		type->tp_adaptive_getattro = base->tp_adaptive_getattro;
		type->tp_adaptive_setattro = base->tp_adaptive_setattro;
		type->tp_adaptive_specializedictaccess = base->tp_adaptive_specializedictaccess;
	}
...
PyObject *
_PyType_LookupCached(PyTypeObject *type, PyObject *name, const unsigned int h, const unsigned int version)
{
...
PyObject *
_PyType_LookupDetail(PyTypeObject *type, PyObject *name, unsigned int* h, unsigned int* version)
{
	Py_ssize_t i, n;
...
_PyType_Lookup(PyTypeObject *type, PyObject *name)
{
    unsigned int h;
	unsigned int version;
	return _PyType_LookupDetail(type, name, &h, &version);
}
