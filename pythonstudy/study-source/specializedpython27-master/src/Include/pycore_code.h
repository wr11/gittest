...
typedef struct {
    int32_t cache_count;
    int32_t instruction_count; /* Force 8 byte size */
} _PyEntryZero;

typedef struct {
    uint16_t original_oparg;
    uint8_t counter;
	uint8_t reserved8;
	uint16_t index;
	uint16_t any_version16;
} _PyAdaptiveEntry;

#define ADAPTIVE_ATTR_MAX_INDEX 0xFFFF // sizeof(index)

typedef struct {
	uint32_t method_type_version;
	uint32_t method_cache_index;
} _PyAttrCache;
...
int _Py_Specialize_StoreSubscr(PyObject *sub, PyObject *container, _Py_CODEUNIT *instr);
