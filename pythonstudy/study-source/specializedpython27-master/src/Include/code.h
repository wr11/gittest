...
typedef unsigned char _Py_CODEUNIT;
#define _Py_OPCODE(pcode) (*(pcode))
#define _Py_OPARG(pcode) (HAS_ARG(*(pcode)) ? OP_ARG(pcode) : 0)
#define _Py_MAKECODEUNIT(pcode, opcode, oparg) { *(pcode) = opcode; *((pcode) + 1) = oparg & 255; *((pcode) + 2) = (oparg >> 8); }
#define _Py_MAKECODEUNIT_NOARG(pcode, opcode) { *(pcode) = opcode; }
